...
export class ReplyUseCase {
  ...
  async createReply(
    ...
    if (type !== "start_test") {
      const test = await this.dataService.tests.get({where : {assignment_id: assignmentId}, relations: ["course"]})
      let courseProgress = await this.dataService.course_progress.get({where: {user: {user_id: studentId}}})
      if (!courseProgress) {
        // @ts-ignore
        courseProgress = this.dataService.course_progress.create({user : student, course : test.course, points : Math.floor(score)})
      } else {
        courseProgress.points += Math.floor(score);
      }
      const isExisting = await this.dataService.replies.get({where : {assignment_id: assignmentId, student: studentId}})
      if (!isExisting) await this.dataService.course_progress.save(courseProgress)
      if (test.is_final && (score / totalPoints * 100) >= 80) {
        ...
        if (currentIndex !== -1 && currentIndex < levels.length - 1) {
          ...
          if (nextCourse && !nextCourse.students.some((s) => s.user_id === studentId)) {
